Feature: Testing report uploads
  ...
  Scenario Outline: Test uploads by creating, uploading, downloading and confirming
    ...
    Examples:
      | Datacapture          | ReportName                  | ReferenceText     | AddNotes                 | response2                                  | response3                             | response4                                        |
      | Test All Controls RM | Report Items - Upload tests | Uploads Reference | Upload items Extra Notes | Multiple more entries are required to test | Third option specifically for testing | Text the same as the button for testing purposes |