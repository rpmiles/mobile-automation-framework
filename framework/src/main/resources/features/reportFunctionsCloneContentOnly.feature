Feature: Cloning Reports
  ...
  Scenario Outline: Clone a report copying with notes only and including rating in multi text item
    ...
    Examples:
      ...
      | Clone Testing - Master | Mobile - Reports - Clone - Content Only | Reference Clone Content Only | true                 | 14 February 2026 | true          | 31 October 2026 | Clone report with content only | false    | false     |
  ...
  Scenario Outline: Confirm each cloned item value is correct
    ...
    And I confirm the uploaded date option "month" is set to "MAY"
    And I confirm the uploaded date option "day" is set to "1"
    And I select the tick or done button
    And I select the item "This is a Switch"
    And I confirm "Switch 2" is a selected option for "This is a Switch"
    And I confirm "Switch 1" is not a selected option for "This is a Switch"
    And I select the tick or done button
    And I select the item "Multi Select Pick List"
    And I confirm "Option 3" is a selected option for "Multi Select Picklist"
    And I confirm "Option 4" is a selected option for "Multi Select Picklist"
    And I select the tick or done button
    ...
    Examples:
      | TargetReport             | ReportName                              | ReferenceText                | SelectInspectionDate | InspectionDate   | SelectDueDate | DueDate         | ExtraNotes                     | AddNotes | AddPhotos |
      | Cloning Testing - Master | Mobile - Reports - Clone - Content Only | Reference Clone Content Only | true                 | 14 February 2026 | true          | 31 October 2025 | Clone report with content only | false    | false     |
